...
public:
   ...
   public:
      ...
      iterator operator ++ (int) { iterator i(_node);
                                   ++_node;
                                   return i; }
      iterator& operator -- () { --_node; return (*this); }
      ...
      iterator operator + (int i) const { iterator it(_node + i);
                                          return it; }
   ...
   void push_back(const T& x) {  if(_size < _capacity)
                                 {
                                    _data[_size] = x;
                                    ++_size;
                                 }
   ...
   void pop_front() {  if(_size)
                       {
                           --_size;
                           _data[0] = _data[_size]; } }
   void pop_back() {  if(_size) --_size; }

   bool erase(iterator pos) { if(!_size)
                                 return false;   
                              --_size;
                              iterator END(end());
                              *pos = *(END);
   ...
   bool erase(const T& x) {  bool hadFound = false;
                              iterator END(end());
